'''
    Edited driver from the graph_recon_DM github repo here: https://github.com/wangjiayuan007/graph_recon_DM
    To use, you must compile the spt_cpp file in the spt_cpp directory.
    See README.md for instructions on how to compile.
'''
...
def cmp_dm_img_grid2D(i_grid2d, i_th, dir_path: str, i_file_name="Temporary", i_subsample=1):
    '''
        Compute the Morse skeleton of a 2d grid (i.e. 2d black-and-white image)

        Args:
            i_grid2d (2d numpy array of floats) : input image
            i_th (float) : threshold for Morse simplification; see paper for description of this threshold
            i_file_name (string) : name of the file. precomputed simplicial complex is saved in a directory named {i_file_name}
                                   this is necessary for the function but can be overwritten later.
            i_subsample (int) : factor by which to subsample {i_grid2d}

        retval:
            o_vert (size V x 2 numpy array of floats) : each row is 2d coordinates of a vertex in the Morse skeleton
            o_edge (size E x 3 numpy array of floats) : first two elements of each row are index of each edge's endpoints in {o_vert}.
                                                         Mitchell doesn't know what third element is.
    '''
    '''
        Mitchell's changes to this funciton:
            - added comments with what I \textit{think} is happening
            - set default value of i_subsample=1
            - commented out line to exit when the Morse skeleton was empty
    '''
    # These paths assume that the repo for this code is in a directory at the same level as the current directory, i.e.
    #   current_dir
    #       current_code.ipynb
    #   graph_recon_dir
    #       graphRecon.py

    # DIR_PATH = "../graph_recon_DM/"
    # DIR_PATH = "graph_recon_DM/"
    
    DIR_PATH = f"{dir_path}/"

    # subsample the input grid
    ...
    # create a dictionary to store the simplicial complex
    if not os.path.exists(DIR_PATH + "dataset/" + i_file_name + "/"):
        os.makedirs(DIR_PATH + "dataset/" + i_file_name + "/")


    outBinary(vert, edge, triangle, DIR_PATH+"dataset/"+i_file_name+'/SC.bin')

    if not os.path.exists(DIR_PATH+"result/"+i_file_name+"/"):
        os.makedirs(DIR_PATH+"result/"+i_file_name+"/")

    # call the cpp function to compute the Morse skeleton
    subprocess.check_call([DIR_PATH+r"spt_cpp/spt_cpp", DIR_PATH+"dataset/"+i_file_name+'/SC.bin', DIR_PATH+"result/"+i_file_name+"/", str(i_th), str(2)])

    o_vert = np.loadtxt(DIR_PATH+"result/"+i_file_name+"/vert.txt")
    o_edge = np.loadtxt(DIR_PATH+"result/"+i_file_name+"/edge.txt")
...
if __name__=='__main__':
    ...
    print(sys.argv)
    # check that the user gave 5 arguments
    if len(sys.argv) != 5:
    ...
    if dimension == 2:
        ## input grid
        # if the simplicial complex has already been built, use the presaved complex
        if os.path.isfile('result/'+file_name+'/presave.bin') and os.path.isfile('dataset/'+file_name+'/SC.bin'):
            ...
            if len(recon_edge)>0:
                ...
                if input_arg=='-g':
                    ...
                    # normalize the filtration to the range [0,1]
        ...
        # the simplicial complex has not been built and the input is a grid
        elif input_arg=='-g':
            # load the image from a 2d text file
            grid2d = np.loadtxt('dataset/'+file_name+'/grid.txt')
            # normalize the grid
            grid2d = grid2d /np.max(np.abs(grid2d))
            # compute the graph from the input grid and threshold
            # output is set of edges and vertices of the graph
            ...
            if len(recon_edge)>0:
                ...
                plt.savefig('result/'+file_name+'/visualization.png', dpi=200)
                plt.show()
        # the simplicial complex has not been built and the input is a SC
        elif input_arg=='-t':
