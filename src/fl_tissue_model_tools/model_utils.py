...
class WarmupSchedule(LearningRateSchedule):
    ...
    def __init__(self, warmup_steps: int, after_warmup_lr: Union[dict, float]):
        ...
        Args:
            ...
            after_warmup_lr (Union[dict, float]):
                LR after warmup. Can be a config for a LearningRateSchedule or a constant.
        """

        self.warmup_steps = int(warmup_steps)

        if isinstance(after_warmup_lr, Number) or isinstance(after_warmup_lr, str):
            ...
            self.after_warmup_init_lr = after_warmup_lr
        else:
            self.after_warmup_lr_config = after_warmup_lr
            self.after_warmup_lr = optimizers.schedules.deserialize(after_warmup_lr)
            self.after_warmup_init_lr = self.after_warmup_lr(0).numpy()

    def __call__(self, step):
        """Compute LR at a given training step."""
        return self._get_lr(tf.cast(step, tf.float32))

    @tf.function
    def _get_lr(self, step: tf.float32) -> tf.float32:
        def get_lr_before_warmup():
            return self.after_warmup_init_lr * (step+1) / self.warmup_steps
        def get_lr_after_warmup():
            return self.after_warmup_lr((step+1) - self.warmup_steps)
